...
def svg_heatmap(
    ...
    if not finite:
        return '<p class="empty">No data yet.</p>'
    bound = max(abs(min(finite)), abs(max(finite)), 1e-9)
    ...
    for r, label in enumerate(row_labels):
        ...
        for c in range(len(col_labels)):
            ...
            color = _diverging_color(value, bound) if value is not None else "var(--gridline)"
...
def _load_data():
    ...
    if NOISE_FLOOR_PATH.exists():
        try:
            sigma = float(json.loads(NOISE_FLOOR_PATH.read_text())["std"])
        except (json.JSONDecodeError, KeyError, ValueError, OSError):
            sigma = None

    return results, all_metrics, latest_ablation, latest_plan, sigma
...
def section_trend(results: List[Dict[str, Any]], sigma: Optional[float]) -> str:
...
def main():
    results, all_metrics, latest_ablation, latest_plan, sigma = _load_data()

    doc = PAGE_TEMPLATE.format(
        generated=time.strftime("%Y-%m-%d %H:%M:%S"),
        trend=section_trend(results, sigma),
        importance=section_importance(all_metrics),
        status=section_status(results),
        importance_evolution=section_importance_evolution(all_metrics),
        heads=section_head_heatmap(latest_ablation),
        search_plan=section_search_plan(latest_plan),
    )

    OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT_PATH.write_text(doc, encoding="utf-8")
    print(f"[visualize_dashboard] Wrote {OUTPUT_PATH} "
          f"({len(results)} results.tsv rows, {len(all_metrics)} reports)")
